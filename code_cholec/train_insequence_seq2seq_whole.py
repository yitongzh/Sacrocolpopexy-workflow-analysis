...
class data_loder(data.Dataset):
    ...
    def sliwin_idx_decoder(self, sliwins, inputs, cur_idx):
        max_seq_len = max([sliwins[i][cur_idx % len(sliwins[i])][-1] + 1 for i, dict in enumerate(inputs)])

        sequence_input_fc = torch.zeros(len(inputs), max_seq_len, 1200)
        sequence_true = torch.zeros(len(inputs), max_seq_len)
        sequence_input_pred = torch.zeros(len(inputs), max_seq_len)
        ...
        for i, dict in enumerate(inputs):
            ...
            sequence_true[i, 0:cur_sliwin[-1] + 1] = torch.tensor(dict['true'][0:cur_sliwin[-1] + 1])
            sequence_input_pred[i, 0:cur_sliwin[-1] + 1] = torch.tensor(dict['pred'][0:cur_sliwin[-1] + 1])
            sequence_clip_num[i, :] = torch.tensor(cur_sliwin)
            # for j, idx in enumerate(cur_sliwin):
            #     sequence_input_fc[i, j, :] = dict['fc_list'][idx]
            for j in range(cur_sliwin[-1] + 1):
                sequence_input_fc[i, j, :] = dict['fc_list'][j]
...
def eval(eval_list):
    ...
    for video in tqdm(eval_list, ncols=80):
        for i in range(len(video)):
            ...
            labels_eval = inputs_dict_eval['true'].long().to(device)
            trg_seq_eval = inputs_dict_eval['pred'].long()
            # clip_num_seq_eval = inputs_dict_eval['clip_num'][:, 0:90].long()
            # clip_num_seq_eval = random_replace(clip_num_seq_eval, 0).long().to(device)
            trg_seq_eval = random_replace(trg_seq_eval, 0).long().to(device)
            with torch.no_grad():
...
if model_type == 'LSTM':
    ...
    # model.load_state_dict(torch.load('./params/params_trans_half.pkl'))
...
print('clip number:', sum(clip_nums)/len(clip_nums))
...
for epoch in range(100):
    ...
    for i in tqdm(range(len(data_sequence)), ncols=80):
        ...
        labels = inputs_dict['true'].long().to(device)
        trg_seq = inputs_dict['pred'].long()
    ...
    y_train_loss, y_train_acc, cm1 = eval(train_sequence_single_ls)
    ...
    anot1 = np.sum(np.diag(cm1)) / 7 * 100
    ...
    vis.line(X=x, Y=np.column_stack((anot1, anot)), win='accuracy', update='append',
