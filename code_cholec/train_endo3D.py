...
for epoch in range(1000):
    ...
    # # indices_list = [[list_40[i]] for i in range(40)]
    # for indices in tqdm(indices_list, ncols=80):
    #     model.train()
    #     inputs = torch.index_select(fcps, 0, torch.tensor(indices)).to(device)
    #     optimizer.zero_grad()
    #     output = model.forward(inputs)
    #     train_loss = [sequence_loss(output[count, 0:labels[value].size(1), :].unsqueeze(0),
    #                   labels[value].to(device), device) for count, value in enumerate(indices)]
    ...
    # Save point
    if y_batch_acc > best_accuracy:
        x = list(np.diag(cm) * 100)
        best_accuracy = y_batch_acc
        torch.save(model.state_dict(), './params/params_LSTM_save_point.pkl')
        print('the current best accuracy without transition phase is: %.3f %%' % y_batch_acc)
