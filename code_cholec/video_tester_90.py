    ...
    for video_name in video_names:
        for extension in extensions:
            ...
            if sequentiaol_model_name == 'transformer':
                ...
                #                                             extension + '.pkl'))
                sequential_model.load_state_dict(torch.load('./params/params_trans_90_noised_w.pkl'))
                slin_sz = 30
            else:
                sequential_model = seq2seq_LSTM(hidden_dim=1200, num_layers=3, tar_seq_dim=7).to(device_s)
                # sequential_model.load_state_dict(torch.load('./params/params_LSTM_90_' +
                #                                             extension + '.pkl'))

                sequential_model.load_state_dict(torch.load('./params/params_LSTM_90_noised_w.pkl'))
            ...
            for i, cur_slwin in enumerate(slinwin_list):
                ...
                # trg_seq = torch.tensor(cur_slwin[0:90]).unsqueeze(0).long()  # the number of current 90 clips
            ...
            correct_pred_eval = (torch.tensor(sequece_label) == torch.tensor(seq_true)).sum().item()
            total_pred_eval = torch.tensor(sequece_label).size(0)
            ...
            results = phase_f1(np.array(seq_true), mode_av)
