#! /usr/bin/env python
from model import Endo3D_1vo, Endo3D_1vo1
...
def main():
    ...
    model = Endo3D_1vo().to(device)

    # load pre-trained parameters
    # Endo3D_state_dict = model.state_dict()
    # pre_state_dict = torch.load('./params/params_endo3d.pkl')
    ...
    model.load_state_dict(torch.load('./params/params_endo3d_1vo.pkl'))
    ...
    evaluation_slot = 50
    rebuild_slot = 7
    sacro = sacro_loder(train_epoch_size=5000, validation_epoch_size=600)
    train_loader = data.DataLoader(sacro, 25, shuffle=True)
    ...
    # optimizer = optim.Adam([{'params': conv_params, 'lr': 1e-8, 'weight_decay': 3e-5},
