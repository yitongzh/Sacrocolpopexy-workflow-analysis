...
def main():
    ...
    pre_state_dict = torch.load('./params/params_c3d_1200.pkl')
    new_state_dict = {k: v for k, v in pre_state_dict.items() if k in Endo3D_state_dict}
    del new_state_dict['fc_phase.weight']
    del new_state_dict['fc_phase.bias']
    ...
    print('loading data')
    start = time.time()
    #sacro = sacro_loder(train_epoch_size=10, validation_epoch_size=10)
    evaluation_slot = 25
    rebuild_slot = 7
    div = 'dataset_div1.json'
    print(div)
    sacro = sacro_loder(train_epoch_size=5000, validation_epoch_size=600, cv_div=div)
